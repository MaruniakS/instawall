class HomeController < ApplicationController
  ...
  def tags
    render :partial => 'shared/tags',:formats => :html
  end

  def get_photos
    tag_name = params[:tag]
    max_id = params[:max_id]
    save_tag(tag_name) if max_id.blank?
    cache = Cache.get_response(current_user.id, request.original_url)
    render json: cache and return if cache
    media = max_id.blank? ? current_user.instagram_client.tag_recent_media(tag_name, count: 5) : current_user.instagram_client.tag_recent_media(tag_name, count: 5, max_id: max_id)
